...
tf.app.flags.DEFINE_integer("word_d", 50, "word vector dimension")
...
def get_embedding(sequences):
    ...
    with tf.variable_scope('local1') as scope:
        ...
        local1 = tf.nn.tanh(tf.matmul(xi, weights) + biases, name=scope.name)
        _activation_summary(local1)

    local1_reshape = tf.reshape(local1, [FLAGS.batch_size,
                                         FLAGS.embed_length, 1024])
...
def inference(page):
    ...
    Returns:
        Logits.
    """
    target_wv, unlabeled_wv, labeled_p = page
